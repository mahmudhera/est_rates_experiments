...
def plot(df, which_rate_to_plot, ax, true_rate):
    ...
    ax.axhline(y=true_rate, color='grey', linestyle='-.', zorder=1, linewidth=0.65)
    
    sns.boxplot(x='ksize', y=which_rate_to_plot, data=df, ax=ax, linewidth=0.5, showfliers=False, width=0.5) 
    ...
    # x and y labels
    ax.set_xlabel('$k$-mer size used in estimators')
    ax.set_ylabel('Estimated ' + pretty_rate_names[which_rate_to_plot])
    
    # set x tick labels at 45 degrees
